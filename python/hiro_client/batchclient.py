...
class HiroBatchRunner:
    ...
    def run(self, attributes: dict) -> Iterator[Tuple[dict, int]]:
        """
...
class CreateEdgesFromSessionRunner(CreateEdgesRunner):
    """
    Create edges between vertices from edges saved in session
    """

    def run_from_session(self) -> Iterator[Tuple[dict, int]]:
        """
        Create a payload from the self.session_data.edge_store and use it to create edges.

        :return: A response dict - usually directly the structure received from the backend.
        """
        for ogit_id, edge_data in self.session_data.edge_store.items():
            for edge in edge_data:
                try:
                    other_ogit_id = self.get_and_check_vertex_id(edge, "vertex_id", "vertex_xid")
                    verb = self.get_and_check(edge, "verb")
                    ...
                    if direction == 'in':
                        attributes = {
                            "to:ogit/_id": ogit_id,
                            "verb": verb,
                            "from:ogit/_id": other_ogit_id
                        }
                    else:
                        attributes = {
                            "from:ogit/_id": ogit_id,
                            "verb": verb,
                            "to:ogit/_id": other_ogit_id
                        }
...
class GraphitBatch:
    ...
    def __init_session(self) -> SessionData:
        ...
        """
        return SessionData() if self.use_xid_cache is True else SessionData.new_with_cache_disabled()

    def create_vertices(self, attributes: dict, session: SessionData = None) -> Iterator[Tuple[dict, int]]:
        """
        ...
        return CreateVerticesRunner(session, self.connection).run(attributes)

    def update_vertices(self, attributes: dict, session: SessionData = None) -> Iterator[Tuple[dict, int]]:
        """
        ...
        if not session:
            session = self.__init_session()
        return UpdateVerticesRunner(session, self.connection).run(attributes)

    def delete_vertices(self, attributes: dict, session: SessionData = None) -> Iterator[Tuple[dict, int]]:
        """
        ...
        if not session:
            session = self.__init_session()
        return DeleteVerticesRunner(session, self.connection).run(attributes)

    def handle_vertices(self, attributes: dict, session: SessionData = None) -> Iterator[Tuple[dict, int]]:
        """
        ...
        if not session:
            session = self.__init_session()
        return HandleVerticesRunner(session, self.connection).run(attributes)

    def create_edges(self, attributes: dict, session: SessionData = None) -> Iterator[Tuple[dict, int]]:
        """
        ...
        if not session:
            session = self.__init_session()
        return CreateEdgesRunner(session, self.connection).run(attributes)

    def delete_edges(self, attributes: dict, session: SessionData = None) -> Iterator[Tuple[dict, int]]:
        """
        ...
        if not session:
            session = self.__init_session()
        return DeleteEdgesRunner(session, self.connection).run(attributes)

    def add_timeseries(self, attributes: dict, session: SessionData = None) -> Iterator[Tuple[dict, int]]:
        """
    ...
    def multi_command(self, command_iter: Iterator[dict]) -> Iterator[Tuple[dict, int]]:
        ...
        handle_session_edges = False
        for command_entry in command_iter:
            for command, attributes in command_entry.items():

                # Special case "handle_vertices_with_edges". This is the same as "handle_vertices", but edge
                # data is transported alongside and has to be handled after "handle_vertices" has finished.
                if command == "handle_vertices_with_edges":
                    handle_session_edges = True
