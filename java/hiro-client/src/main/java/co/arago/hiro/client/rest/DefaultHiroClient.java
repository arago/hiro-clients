...
public class DefaultHiroClient implements HiroClient {
  ...
  public DefaultHiroClient(String restApiUrl, TokenProvider tokenProvider, AsyncHttpClient client, boolean trustAllCerts, Level debugLevel, int timeout) {
  ...
  public List<Map> getVertexNeighbours(String vertexId, String edgeType, Direction dir) {
    ...
    String result = restClient.get(HiroCollections.newList(notEmpty(vertexId, "vertexId"), notEmpty(edgeType, "edgeType")), params);
    return Helper.parseItemList(result);
  }
  ...
  public List<TimeseriesValue> getTsValues(String tsNodeId, long from, long to, Map<String, String> queryParams) {
    ...
    Map tmpResult = Helper.parseJsonBody(restClient.get(HiroCollections.newList(notEmpty(tsNodeId, "tsNodeId"), URL_PATH_VALUES), queryParams));
  ...
  public void updateContent(String attachmentNodeId, InputStream is) {
    restClient.postBinary(HiroCollections.newList(notEmpty(attachmentNodeId, "attachmentNodeId"), URL_PATH_ATTACHMENT), is);
  }
  ...
  public Map setVariable(Map kwargs) {
    String result = restClient.put(HiroCollections.newList(URL_PATH_VARIABLES), Helper.composeJson(notNull(kwargs, "kwargs")), null);
    return Helper.parseJsonBody(result);
  }
  ...
  public void getEventStream(Map<String, String> requestParameters, Listener<String> dataListener, Listener<String> logListener) {
    ...
    try {
      ...
      URI uri = new URI(this.restApiUrl.replace("http", "ws") + "/_events?" + prepareEventStreamPArams(requestParameters));
      ...
      try (Session session = webSocketClient.connect(new SimpleWsListener(requestParameters.get("filter"), dataListener, logListener), uri, clientUpgradeRequest).get()) {
