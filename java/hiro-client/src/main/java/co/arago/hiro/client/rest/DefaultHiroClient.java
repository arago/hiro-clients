...
public class DefaultHiroClient implements HiroClient {
    ...
    public DefaultHiroClient(String restApiUrl, TokenProvider tokenProvider, AsyncHttpClient client,
        ...
        if (apiVersion != null && !apiVersion.isEmpty()) {
            ...
            apiVersionInfo.put(API_SUFFIX, apiVersion);
        } else if (apiVersion != null && apiVersion.isEmpty()) {
            apiPath = "";// 6.0 graph. deprecated
            apiVersionInfo.put(API_SUFFIX, "");
        }
    ...
    public void updateTsValuesSynchronous(String tsNodeId, List<TimeseriesValue> values) {
        restClient.post(HiroCollections.newList(notEmpty(tsNodeId, "tsNodeId"), URL_PATH_VALUES),
                Helper.composeJson(notEmpty(values, "values")), HiroCollections.newMap(QUERY_PARAM_TS_SYNC, "true"));
    }

    @Override
    public void updateTsValues(String tsNodeId, List<TimeseriesValue> values, String ttl) {
        restClient.post(HiroCollections.newList(notEmpty(tsNodeId, "tsNodeId"), URL_PATH_VALUES),
                Helper.composeJson(notEmpty(values, "values")), HiroCollections.newMap(QUERY_PARAM_TS_TTL, ttl));
    }
    ...
    public void updateTsValuesSynchronous(String tsNodeId, List<TimeseriesValue> values, String ttl) {
        restClient.post(HiroCollections.newList(notEmpty(tsNodeId, "tsNodeId"), URL_PATH_VALUES),
                Helper.composeJson(notEmpty(values, "values")),
                HiroCollections.newMap(QUERY_PARAM_TS_SYNC, "true", QUERY_PARAM_TS_TTL, ttl));
    }
    ...
    public Map<Long, String> getTsValueHistory(String tsNodeId, long timestamp) {
        ...
        return Helper.parseJsonBody(restClient.get(
                HiroCollections.newList(notEmpty(tsNodeId, "tsNodeId"), URL_PATH_VALUES, URL_PATH_HISTORY), params));
    }

    @Override
    public String updateContent(String attachmentNodeId, InputStream is) {
        return (String) Helper.parseJsonBody(restClient.postBinary(
    ...
    public void getEventStream(Map<String, String> requestParameters, Listener<String> dataListener,
        ...
        builder.setClient(restClient.client()).setRestApiUrl(restApiUrl).setDebugRest(debugLevel).setTimeout(timeout)
                .setTokenProvider(tokenProvider).setTrustAllCerts(trustAllCerts);

        try (WebSocketClient ws = builder.makeWebSocketClient(ClientBuilder.WebsocketType.Event,
                prepareEventStreamParams(requestParameters), dataListener, logListener,
                new SimpleWsListener(requestParameters.get("filter")));) {
